...
class JoystickProcessor:
    ...
    def _gear_multiplier(self, value):
        ...
        # A3/B3 gear positions (min -> max): 0.5, 0, -0.5, -1
        # Use tolerant ranges for noisy readings.
        if value >= 0.25:
            return 0.5
        if value >= -0.25:
            return 0.8
    ...
    def parse_joystick_string(self, data_str):
        ...
        for axis, num, value in matches:
            ...
            try:
                ...
                logger.warning(f"??????? {param_name}:{value}")
    ...
    def process_data(self, latest_data):
        ...
        try:
            # 1. 回转控制 (X1)
            if 'X1' in params:
                if abs(params['X1']) > 0 or params['X1'] == 0:
        ...
        try:
             # 2. 小臂控制 (Y1)
            if 'Y1' in params:
                if abs(params['Y1']) > 0 or params['Y1'] == 0:
        ...
        try:
            # 3. 翻斗控制 (X2)
            if 'X2' in params:
                if abs(params['X2']) > 0 or params['X2'] == 0:
        ...
        try:
            # 4. 大臂控制 (Y2)
            if 'Y2' in params:
                if abs(params['Y2']) > 0 or params['Y2'] == 0:
        ...
        try:
            # 6. 转向舵机控制 (Z2)
            if 'Z2' in params:
                if abs(params['Z2']) > 0 or params['Z2'] == 0:
    ...
    def print_data(self, latest_data):
        ...
        try:
            ...
            print(f"\n[{time_str}] 收到数据:")
            print("-" * 50)
            print(latest_data.get("formatted_data", "???"))
            print("-" * 50)
