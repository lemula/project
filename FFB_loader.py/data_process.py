...
class DataProcessor:
    ...
    def _has_command(self, value):
        value = 0.0 if value is None else value
        return abs(value) >= COMMAND_DEADBAND

    def _pump_angle_for_commands(self, steer_input, bucket_input, boom_input):
        pump_active = any(
            (
                self._has_command(steer_input),
                self._has_command(bucket_input),
                self._has_command(boom_input),
            )
        )
        self.pump_on = pump_active
        return PUMP_ACTIVE_ANGLE if pump_active else PUMP_NEUTRAL

    def angle_control(self, steer_input):
    ...
    def process(self, data):
        ...
        pump_angle = self._pump_angle_for_commands(steer, stick_bucket, stick_boom)

        try:
            self.angle_control(steer)
