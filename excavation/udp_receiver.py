...
class UdpJoystickReceiver:
    def __init__(self, listen_ip='192.168.2.14', listen_port=8888, on_data=None, on_print=None, print_interval=1.0):
    ...
    def start(self):
        ...
        logger.info(f"监听 {self.listen_address} 接收中...")
            ...
            while True:
                ...
                try:
                    ...
                    if json_data.get('type') == 'handshake':
                        self.connected = True
                        logger.info(f"已连接 {addr} 设备")
                        response = json.dumps({"status": "ready"})
                        self.sock.sendto(response.encode('utf-8'), addr)
                        logger.info(f"已向 {addr} 发送就绪")
                    else:
                        self.connected = True
                        if self.on_data:
                    ...
                    logger.error(f"无法解析 JSON 数据: {data.decode('utf-8')}")
